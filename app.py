...
def register():
    ...
    if len(phone) != 10:
        return jsonify(success=False, message="Enter valid 10 digit mobile number"), 400
...
def place_order():
    ...
    payment_mode = str(data.get("payment_mode", "COD"))
    instructions = str(data.get("instructions", ""))

    if not customer_id or not restaurant_id:
        return jsonify(success=False, message="Login and restaurant required"), 400

    if not items:
        return jsonify(success=False, message="Cart is empty"), 400

    if len(address) < 5:
        return jsonify(success=False, message="Enter delivery address"), 400
    ...
    for item in items:
        ...
        valid_items.append((food, qty))
        item_total += float(food["price"]) * qty

    if not valid_items:
        conn.close()
        return jsonify(success=False, message="No valid food items"), 400

    discount = 0

    coupon_code = str(data.get("coupon_code", "")).upper().strip()
    ...
    delivery_fee = 0 if item_total >= 500 else 25
    platform_fee = 5
...
def whatsapp_message(order_id):
    ...
    for item in items:
        text += f"{item['food_name']} x {item['quantity']}\n"
...
def restaurant_login():
    ...
    SELECT * FROM restaurants
    WHERE phone=? AND password=? AND active=1
...
def restaurant_add_food():
    ...
    if not restaurant_id or not name or price <= 0:
        return jsonify(message="Invalid food details"), 400
    ...
    cur.execute("""
    INSERT INTO foods(
        restaurant_id,name,price,type,description
    )
    VALUES (?,?,?,?,?)
    """, (
        restaurant_id,
...
def delivery_register():
    ...
    if len(name) < 2 or len(phone) != 10 or len(password) < 4:
        return jsonify(message="Enter valid details"), 400
    ...
    return jsonify(success=True, partner=dict(partner))
...
def update_delivery_status(order_id):
    ...
    valid = ["Picked Up", "On the Way", "Delivered"]
...
def admin_login():
    ...
    admin_pass = os.environ.get("ADMIN_PASSWORD", "swipto123")
...
def admin_orders():
    ...
    SELECT
        ...
        customers.phone customer_phone,
        restaurants.name restaurant_name,
        delivery_partners.name delivery_partner_name
    FROM orders
    LEFT JOIN customers ON customers.id=orders.customer_id
    LEFT JOIN restaurants ON restaurants.id=orders.restaurant_id
...
def admin_add_coupon():
    ...
    try:
        ...
        minimum = float(data.get("minimum_order", 0))
    except:
        return jsonify(message="Invalid values"), 400

    if not code or value <= 0:
        return jsonify(message="Enter valid coupon"), 400
